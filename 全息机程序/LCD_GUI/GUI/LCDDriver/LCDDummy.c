...
{
	setPoint(x,y,PixelIndex);//对应一个坐标上画一个点
}
...
//    for (; x0 <= x1; x0++) {
//      LCD_L0_SetPixelIndex(x0, y0, LCD_COLORINDEX);
//    }
		//Lcd_ColorBox(x0,y0,x1-x0,1,LCD_COLORINDEX);
		DrawHLine(x0,y0,x1,LCD_COLORINDEX);
  }
}
...
//    }
		//Lcd_ColorBox(x0,y0,1,y1-y0,LCD_COLORINDEX);
		DrawVLine(x0,y0,y1,LCD_COLORINDEX);
  }
}
...
//  }
	...
	FillRect(x0,y0,x1,y1,LCD_COLORINDEX);
}
